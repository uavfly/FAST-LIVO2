...
LIVMapper::LIVMapper(rclcpp::Node::SharedPtr &nh, std::string node_name)
    : nh(nh),
      extT(0, 0, 0),
      extR(M3D::Identity())
{
  ...
  path.header.stamp = nh->get_clock()->now();
...
void LIVMapper::readParameters(rclcpp::Node::SharedPtr &nh)
{
  ...
  nh->declare_parameter<bool>("common.ros_driver_bug_fix", false);
  nh->declare_parameter<int>("common.img_en", 1);
  nh->declare_parameter<int>("common.lidar_en", 1);
  nh->declare_parameter<std::string>("common.img_topic", "/left_camera/image");

  nh->declare_parameter<bool>("vio.normal_en", true);
  nh->declare_parameter<bool>("vio.inverse_composition_en", false);
  nh->declare_parameter<int>("vio.max_iterations", 5);
  nh->declare_parameter<int>("vio.img_point_cov", 100);
  nh->declare_parameter<bool>("vio.raycast_en", false);
  nh->declare_parameter<bool>("vio.exposure_estimate_en", true);
  nh->declare_parameter<double>("vio.inv_expo_cov", 0.1);
  nh->declare_parameter<int>("vio.grid_size", 5);
  nh->declare_parameter<int>("vio.grid_n_height", 17);
  nh->declare_parameter<int>("vio.patch_pyrimid_level", 4);
  nh->declare_parameter<int>("vio.patch_size", 8);
  nh->declare_parameter<int>("vio.outlier_threshold", 100);

  nh->declare_parameter<double>("time_offset.exposure_time_init", 0.0);
  nh->declare_parameter<double>("time_offset.img_time_offset", 0.0);
  nh->declare_parameter<double>("time_offset.imu_time_offset", 0.0);
  nh->declare_parameter<double>("time_offset.lidar_time_offset", 0.0);
  nh->declare_parameter<bool>("uav.imu_rate_odom", false);
  nh->declare_parameter<bool>("uav.gravity_align_en", false);

  nh->declare_parameter<std::string>("evo.seq_name", "01");
  nh->declare_parameter<bool>("evo.pose_output_en", false);
  nh->declare_parameter<double>("imu.gyr_cov", 1.0);
  nh->declare_parameter<double>("imu.acc_cov", 1.0);
  nh->declare_parameter<int>("imu.imu_int_frame", 30);
  nh->declare_parameter<bool>("imu.imu_en", true);
  nh->declare_parameter<bool>("imu.gravity_est_en", true);
  nh->declare_parameter<bool>("imu.ba_bg_est_en", true);

  nh->declare_parameter<double>("preprocess.blind", 0.01);
  nh->declare_parameter<double>("preprocess.filter_size_surf", 0.5);
  nh->declare_parameter<bool>("preprocess.hilti_en", false);
  nh->declare_parameter<int>("preprocess.lidar_type", AVIA);
  nh->declare_parameter<int>("preprocess.scan_line",6);
  nh->declare_parameter<int>("preprocess.point_filter_num", 3);
  nh->declare_parameter<bool>("preprocess.feature_extract_enabled", false);

  nh->declare_parameter<int>("pcd_save.interval", -1);
  nh->declare_parameter<bool>("pcd_save.pcd_save_en", false);
  nh->declare_parameter<bool>("pcd_save.colmap_output_en", false);
  nh->declare_parameter<double>("pcd_save.filter_size_pcd", 0.5);
  nh->declare_parameter<vector<double>>("extrin_calib.extrinsic_T", vector<double>{});
  nh->declare_parameter<vector<double>>("extrin_calib.extrinsic_R", vector<double>{});
  nh->declare_parameter<vector<double>>("extrin_calib.Pcl", vector<double>{});
  nh->declare_parameter<vector<double>>("extrin_calib.Rcl", vector<double>{});
  nh->declare_parameter<double>("debug.plot_time", -10);
  nh->declare_parameter<int>("debug.frame_cnt", 6);

  nh->declare_parameter<double>("publish.blind_rgb_points", 0.01);
  nh->declare_parameter<int>("publish.pub_scan_num", 1);
  nh->declare_parameter<bool>("publish.pub_effect_point_en", false);
  nh->declare_parameter<bool>("publish.dense_map_en", false);

  nh->get_parameter("common.lid_topic", lid_topic);
  nh->get_parameter("common.imu_topic", imu_topic);
  nh->get_parameter("common.ros_driver_bug_fix", ros_driver_fix_en);
  nh->get_parameter("common.img_en", img_en);
  nh->get_parameter("common.lidar_en", lidar_en);
  nh->get_parameter("common.img_topic", img_topic);

  nh->get_parameter("vio.normal_en", normal_en);
  nh->get_parameter("vio.inverse_composition_en", inverse_composition_en);
  nh->get_parameter("vio.max_iterations", max_iterations);
  nh->get_parameter("vio.img_point_cov", IMG_POINT_COV);
  nh->get_parameter("vio.raycast_en", raycast_en);
  nh->get_parameter("vio.exposure_estimate_en", exposure_estimate_en);
  nh->get_parameter("vio.inv_expo_cov", inv_expo_cov);
  nh->get_parameter("vio.grid_size", grid_size);
  nh->get_parameter("vio.grid_n_height", grid_n_height);
  nh->get_parameter("vio.patch_pyrimid_level", patch_pyrimid_level);
  nh->get_parameter("vio.patch_size", patch_size);
  nh->get_parameter("vio.outlier_threshold", outlier_threshold);

  nh->get_parameter("time_offset.exposure_time_init", exposure_time_init);
  nh->get_parameter("time_offset.img_time_offset", img_time_offset);
  nh->get_parameter("time_offset.imu_time_offset", imu_time_offset);
  nh->get_parameter("time_offset.lidar_time_offset", lidar_time_offset);
  nh->get_parameter("uav.imu_rate_odom", imu_prop_enable);
  nh->get_parameter("uav.gravity_align_en", gravity_align_en);

  nh->get_parameter("evo.seq_name", seq_name);
  nh->get_parameter("evo.pose_output_en", pose_output_en);
  nh->get_parameter("imu.gyr_cov", gyr_cov);
  nh->get_parameter("imu.acc_cov", acc_cov);
  nh->get_parameter("imu.imu_int_frame", imu_int_frame);
  nh->get_parameter("imu.imu_en", imu_en);
  nh->get_parameter("imu.gravity_est_en", gravity_est_en);
  nh->get_parameter("imu.ba_bg_est_en", ba_bg_est_en);

  nh->get_parameter("preprocess.blind", p_pre->blind);
  nh->get_parameter("preprocess.filter_size_surf", filter_size_surf_min);
  nh->get_parameter("preprocess.hilti_en", hilti_en);
  nh->get_parameter("preprocess.lidar_type", p_pre->lidar_type);
  nh->get_parameter("preprocess.scan_line", p_pre->N_SCANS);
  nh->get_parameter("preprocess.point_filter_num", p_pre->point_filter_num);
  nh->get_parameter("preprocess.feature_extract_enabled", p_pre->feature_enabled);

  nh->get_parameter("pcd_save.interval", pcd_save_interval);
  nh->get_parameter("pcd_save.pcd_save_en", pcd_save_en);
  nh->get_parameter("pcd_save.colmap_output_en", colmap_output_en);
  nh->get_parameter("pcd_save.filter_size_pcd", filter_size_pcd);
  nh->get_parameter("extrin_calib.extrinsic_T", extrinT);
  nh->get_parameter("extrin_calib.extrinsic_R", extrinR);
  nh->get_parameter("extrin_calib.Pcl", cameraextrinT);
  nh->get_parameter("extrin_calib.Rcl", cameraextrinR);
  nh->get_parameter("debug.plot_time", plot_time);
  nh->get_parameter("debug.frame_cnt", frame_cnt);

  nh->get_parameter("publish.blind_rgb_points", blind_rgb_points);
  nh->get_parameter("publish.pub_scan_num", pub_scan_num);
  nh->get_parameter("publish.pub_effect_point_en", pub_effect_point_en);
  nh->get_parameter("publish.dense_map_en", dense_map_en);

  p_pre->blind_sqr = p_pre->blind * p_pre->blind;
}

void LIVMapper::initializeComponents(rclcpp::Node::SharedPtr &nh) 
{
  ...
  if (!vk::camera_loader::loadFromRosNs(nh, "camera", vio_manager->cam)) throw std::runtime_error("Camera model not correctly specified.");
...
{
  rclcpp::QoS qos(200000);
  if(p_pre->lidar_type == AVIA) 
  {
      sub_pcl_livox = nh->create_subscription<livox_ros_driver2::msg::CustomMsg>(lid_topic, qos, std::bind(&LIVMapper::livox_pcl_cbk, this, std::placeholders::_1));
  } 
  else 
  {
      sub_pcl_pc2 = nh->create_subscription<sensor_msgs::msg::PointCloud2>(lid_topic, qos, std::bind(&LIVMapper::standard_pcl_cbk, this, std::placeholders::_1));
  }
  ...
  plane_pub = nh->create_publisher<visualization_msgs::msg::Marker>("/planner_normal", 10);
  voxel_pub = nh->create_publisher<visualization_msgs::msg::MarkerArray>("/voxels", 10);
...
{    
  ...
  if (pose_output_en) 
  {
    ...
    if (!pos_opend) 
    {
      ...
      pos_opend = true;
      if (!evoFile.is_open()) RCLCPP_ERROR(nh->get_logger(), "open fail\n");
    } 
    else 
    {
...
void LIVMapper::run(rclcpp::Node::SharedPtr &nh) 
{
...
void LIVMapper::imu_prop_callback()
{
  ...
  if (imu_prop_enable && !prop_imu_buffer.empty())
  {
    ...
    if (state_update_flg)
    {
      ...
      while ((!prop_imu_buffer.empty() && get_time_now(prop_imu_buffer.front().header.stamp) < latest_ekf_time))
      {
      ...
      {
        double t_from_lidar_end_time = get_time_now(prop_imu_buffer[i].header.stamp) - latest_ekf_time;
    ...
    else
    {
      ...
      double t_from_lidar_end_time = get_time_now(newest_imu.header.stamp) - latest_ekf_time;
      double dt = t_from_lidar_end_time - last_t_from_lidar_end_time;
      prop_imu_once(imu_propagate, dt, acc_imu, omg_imu);
      last_t_from_lidar_end_time = t_from_lidar_end_time;
    }
...
void LIVMapper::standard_pcl_cbk(const sensor_msgs::msg::PointCloud2::ConstSharedPtr &msg)
{
  ...
  double cur_head_time = get_time_now(msg->header.stamp) + lidar_time_offset;
  // cout<<"got feature"<<endl;
  if (cur_head_time < last_timestamp_lidar)
  {
    RCLCPP_ERROR(nh->get_logger(), "lidar loop back, clear buffer");
    lid_raw_data_buffer.clear();
  }
  // RCLCPP_INFO(nh->get_logger(), "get point cloud at time: %.6f", get_time_now(msg->header.stamp));
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::msg::CustomMsg::ConstSharedPtr &msg_in)
{
  ...
  // if ((abs(get_time_now(msg->header.stamp) - last_timestamp_lidar) > 0.2 && last_timestamp_lidar > 0) || sync_jump_flag)
  // {
  //   RCLCPP_WARN(nh->get_logger(), "lidar jumps %.3f\n", get_time_now(msg->header.stamp) - last_timestamp_lidar);
  //   sync_jump_flag = true;
  //   msg->header.stamp = ros::Time().fromSec(last_timestamp_lidar + 0.1);
  // }
  if (abs(last_timestamp_imu - get_time_now(msg->header.stamp)) > 1.0 && !imu_buffer.empty())
  {
    double timediff_imu_wrt_lidar = last_timestamp_imu - get_time_now(msg->header.stamp);
  ...
  double cur_head_time = get_time_now(msg->header.stamp);
  RCLCPP_INFO(nh->get_logger(), "Get LiDAR, its header time: %.6f", cur_head_time);
  if (cur_head_time < last_timestamp_lidar)
  {
    RCLCPP_ERROR(nh->get_logger(), "lidar loop back, clear buffer");
    lid_raw_data_buffer.clear();
  ...
  if (!ptr || ptr->empty()) {
    RCLCPP_ERROR(nh->get_logger(), "Received an empty point cloud");
    mtx_buffer.unlock();
    return;
  }
...
void LIVMapper::imu_cbk(const sensor_msgs::msg::Imu::ConstSharedPtr &msg_in)
{
  ...
  // RCLCPP_INFO(nh->get_logger(), "get imu at time: %.6f", get_time_now(msg_in->header.stamp));
  sensor_msgs::msg::Imu::SharedPtr msg(new sensor_msgs::msg::Imu(*msg_in));
  msg->header.stamp = get_ros_time(get_time_now(msg->header.stamp) - imu_time_offset);
  double timestamp = get_time_now(msg->header.stamp);

  if (fabs(last_timestamp_lidar - timestamp) > 0.5 && (!ros_driver_fix_en))
  {
    RCLCPP_WARN(nh->get_logger(), "IMU and LiDAR not synced! delta time: %lf .\n", last_timestamp_lidar - timestamp);
  }
  ...
  if (last_timestamp_imu > 0.0 && timestamp < last_timestamp_imu)
  {
    mtx_buffer.unlock();
    sig_buffer.notify_all();
    RCLCPP_ERROR(nh->get_logger(), "imu loop back, offset: %lf \n", last_timestamp_imu - timestamp);
    return;
  }
  ...
  //   RCLCPP_WARN(nh->get_logger(), "imu time stamp Jumps %0.4lf seconds \n", timestamp - last_timestamp_imu);
  //   mtx_buffer.unlock();
  //   sig_buffer.notify_all();
  //   return;
  // }
...
void LIVMapper::img_cbk(const sensor_msgs::msg::Image::ConstSharedPtr &msg_in)
{
  ...
  // {
  //   RCLCPP_WARN(nh->get_logger(), "img jumps %.3f\n", msg->header.stamp.toSec() - last_timestamp_img);
  //   sync_jump_flag = true;
  //   msg->header.stamp = get_ros_time(last_timestamp_img + 0.1);
  // }
  ...
  double msg_header_time = get_time_now(msg->header.stamp) + img_time_offset;
  if (abs(msg_header_time - last_timestamp_img) < 0.001) return;
  RCLCPP_INFO(nh->get_logger(), "Get image, its header time: %.6f", msg_header_time);
  if (last_timestamp_lidar < 0) return;

  if (msg_header_time < last_timestamp_img)
  {
    RCLCPP_ERROR(nh->get_logger(), "image loop back. \n");
    return;
  }
  ...
  {
    RCLCPP_WARN(nh->get_logger(), "Image need Jumps: %.6f", img_time_correct);
    mtx_buffer.unlock();
    sig_buffer.notify_all();
    return;
  }
  ...
  // RCLCPP_INFO(nh->get_logger(), "Correct Image time: %.6f", img_time_correct);
...
bool LIVMapper::sync_packages(LidarMeasureGroup &meas)
{
  ...
  case ONLY_LIO:
  {
    ...
    {
      if (get_time_now(imu_buffer.front()->header.stamp) > meas.lidar_frame_end_time) break;
  ...
  case LIVO:
  {
    ...
    case VIO:
    {
      ...
      double imu_newest_time = get_time_now(imu_buffer.back()->header.stamp);

      if (img_capture_time < meas.last_lio_update_time + 0.00001)
      {
        img_buffer.pop_front();
        img_time_buffer.pop_front();
        RCLCPP_ERROR(nh->get_logger(), "[ Data Cut ] Throw one image frame! \n");
        return false;
      }

      if (img_capture_time > lid_newest_time || img_capture_time > imu_newest_time)
      {
        // RCLCPP_ERROR(nh->get_logger(), "lost first camera frame");
      ...
      while (!imu_buffer.empty())
      {
        if (get_time_now(imu_buffer.front()->header.stamp) > m.lio_time) break;

        if (get_time_now(imu_buffer.front()->header.stamp) > meas.last_lio_update_time) m.imu.push_back(imu_buffer.front());
    ...
    case LIO:
    {
      ...
      // printf("[ Data Cut ] VIO \n");
      meas.measures.clear();
      double imu_time = get_time_now(imu_buffer.front()->header.stamp);
  ...
  RCLCPP_ERROR(nh->get_logger(), "out sync");
}
...
void LIVMapper::publish_img_rgb(const image_transport::Publisher &pubImage, VIOManagerPtr vio_manager)
{
  ...
  out_msg.header.stamp = nh->get_clock()->now();
...
void LIVMapper::publish_frame_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudFullRes, VIOManagerPtr vio_manager)
{
  ...
  laserCloudmsg.header.stamp = nh->get_clock()->now(); //.fromSec(last_timestamp_lidar);
...
void LIVMapper::publish_visual_sub_map(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubSubVisualMap)
{
  ...
  *sub_pcl_visual_map_pub = *laserCloudFullRes;
  if (1)
  {
    ...
    laserCloudmsg.header.stamp = nh->get_clock()->now();
    laserCloudmsg.header.frame_id = "camera_init";
...
void LIVMapper::publish_effect_world(const rclcpp::Publisher<sensor_msgs::msg::PointCloud2>::SharedPtr &pubLaserCloudEffect, const std::vector<PointToPlane> &ptpl_list)
{
  ...
  laserCloudFullRes3.header.stamp = nh->get_clock()->now();
...
void LIVMapper::publish_odometry(const rclcpp::Publisher<nav_msgs::msg::Odometry>::SharedPtr &pubOdomAftMapped)
{
  ...
  odomAftMapped.header.stamp = nh->get_clock()->now(); //.ros::Time()fromSec(last_timestamp_lidar);
