...
{
  RCLCPP_WARN(rclcpp::get_logger("IMU_Processing"), "Reset ImuProcess");
  mean_acc = V3D(0, 0, -1.0);
  mean_gyr = V3D(0, 0, 0);
  angvel_last = Zero3d;
  imu_need_init = true;
...
void ImuProcess::UndistortPcl(LidarMeasureGroup &lidar_meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  const double &imu_beg_time = get_time_now(v_imu.front()->header.stamp);
  const double &imu_end_time = get_time_now(v_imu.back()->header.stamp);
  ...
  if (!imu_time_init)
  {
    // imu_time = get_time_now(v_imu.front()->header.stamp) - first_lidar_time;
  ...
  else
  {
    ...
    // RCLCPP_ERROR(rclcpp::get_logger("IMU_Processing"), "tau: %.6f !!!!!!", tau);
  }
  // state_inout.cov(6, 6) = 0.01;

  // RCLCPP_ERROR(rclcpp::get_logger("IMU_Processing"), "lidar_meas.lio_vio_flg");
  ...
  case VIO:
    ...
    for (int i = 0; i < v_imu.size() - 1; i++)
    {
      ...
      if (get_time_now(tail->header.stamp) < prop_beg_time) continue;
      ...
      // #ifdef DEBUG_PRINT
      fout_imu << setw(10) << get_time_now(head->header.stamp) - first_lidar_time << " " << angvel_avr.transpose() << " " << acc_avr.transpose() << endl;
      // #endif

      // imu_time = get_time_now(head->header.stamp) - first_lidar_time;

      angvel_avr -= state_inout.bias_g;
      acc_avr = acc_avr * G_m_s2 / mean_acc.norm() - state_inout.bias_a;

      if (get_time_now(head->header.stamp) < prop_beg_time)
      {
        // printf("00 \n");
        dt = get_time_now(tail->header.stamp) - last_prop_end_time;
        offs_t = get_time_now(tail->header.stamp) - prop_beg_time;
      }
      else if (i != v_imu.size() - 2)
      {
        // printf("11 \n");
        dt = get_time_now(tail->header.stamp) - get_time_now(head->header.stamp);
        offs_t = get_time_now(tail->header.stamp) - prop_beg_time;
      }
      else
      {
        // printf("22 \n");
        dt = prop_end_time - get_time_now(head->header.stamp);
