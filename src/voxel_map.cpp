...
void loadVoxelConfig(rclcpp::Node::SharedPtr nh, VoxelMapConfig &voxel_config)
{
  ...
  nh->declare_parameter<bool>("publish.pub_plane_en", false);
  nh->declare_parameter<int>("lio.max_layer", 1);
  nh->declare_parameter<double>("lio.voxel_size", 0.5);
  nh->declare_parameter<double>("lio.min_eigen_value", 0.01);
  nh->declare_parameter<double>("lio.sigma_num", 3);
  nh->declare_parameter<double>("lio.beam_err", 0.02);
  nh->declare_parameter<double>("lio.dept_err", 0.05);

  nh->declare_parameter<std::vector<int>>("lio.layer_init_num", std::vector<int>{5,5,5,5,5}); 
  nh->declare_parameter<int>("lio.max_points_num", 50);
  nh->declare_parameter<int>("lio.min_iterations", 5);
  nh->declare_parameter<bool>("local_map.map_sliding_en", false);
  nh->declare_parameter<int>("local_map.half_map_size", 100);
  nh->declare_parameter<double>("local_map.sliding_thresh", 8.0);
  ...
  nh->get_parameter("lio.dept_err", voxel_config.dept_err_);
  //nh->get_parameter("lio.layer_init_num", voxel_config.layer_init_num_);
  
  std::vector<int64_t> temp_vec;
  if (nh->get_parameter("lio.layer_init_num", temp_vec)) {
      // 转换为int
      voxel_config.layer_init_num_.clear();
      voxel_config.layer_init_num_.reserve(temp_vec.size());
      for (const auto& val : temp_vec) {
          voxel_config.layer_init_num_.push_back(static_cast<int>(val));
      }
  } else {
      // 如果获取失败，使用默认值
      RCLCPP_WARN(nh->get_logger(), "Failed to get parameter 'lio.layer_init_num', using default value.");
      // 注意：这里假设在declare_parameter时已经设置了默认值，但为了安全，我们可以再设置一次
